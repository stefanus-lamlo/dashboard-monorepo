...
<!--
Source: https://github.com/claude-office-skills/skills/blob/main/ppt-visual/SKILL.md
Installed into this project on 2026-08-02. The original file's frontmatter was
missing its opening `---` delimiter (comment banner + loose YAML before a lone
closing `---`), which Claude Code's skill loader requires to parse name/description
- fixed here without altering any of the original guidance content below.

The original also references an "office-mcp" MCP server (create_pptx, md_to_pptx
tools) that isn't installed in this environment. This repo already has a working
PPTX generator (apps/api/src/routes/documents.ts, via pptxgenjs) - use this skill
purely as design guidance (palettes, layout patterns, typography) to inform how
that code builds slides, not as a dependency on the missing MCP tools.
-->
